  ...
  return (
    <section className="border-b border-border py-14 lg:py-20">
      <div className="shell">
        ...
        <div className="flex flex-wrap items-center gap-x-10 gap-y-6 border-t border-border pt-8">
          {brands.map((b) => (
            <Link
              key={b.slug}
              to={`/search?brand=${b.slug}`}
              className="link-underline font-display text-xl font-medium text-text-secondary transition-colors hover:text-text sm:text-2xl"
            >
              {b.name}
            </Link>
          ))}
