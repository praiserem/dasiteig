        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {editorial.map((a) => (
            <Link key={a.slug} to={`/journal/${a.slug}`} className="group flex flex-col">
              <div className="grid grid-cols-2 gap-0.5 overflow-hidden rounded-xl border border-border bg-modal">
                <ProductArt
                  art={a.artA.art}
              ...
              <h3 className="mt-1.5 font-display text-[18px] font-medium leading-snug text-text">{a.title}</h3>
              <p className="mt-2 text-[14px] text-text-secondary">{a.description}</p>
              <div className="mt-3 flex items-center justify-between text-[13px] text-text-secondary">
                <span>{a.readTime}</span>
                <ArrowRight
                  size={15}
                  className="text-text-secondary transition-transform duration-200 ease-fast group-hover:translate-x-1 group-hover:text-accent"
                />
              </div>
            </Link>
          ))}
